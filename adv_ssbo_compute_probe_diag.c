...
static const uint32_t ssbo_write_spv[] = {
   ...
   0x00060041, 0x00000008, 0x0000000c,
...
run_child(const char *driver_path, uint64_t fence_timeout_ns)
{
   ...
   probe_log =
      fopen("/data/local/tmp/xclipse940/ssbo_compute_probe.log", "w");
   if (probe_log) {
      setvbuf(probe_log, NULL, _IONBF, 0);
      persist_mark("START ssbo_compute_probe");
   }

   printf("=== Xclipse 940 RADV minimal SSBO compute probe ===\n");
   printf("driver: %s\n", driver_path);
...
main(int argc, char **argv)
{
   ...
   for (;;) {
      ...
      if (r == child) {
         FILE *f =
            fopen("/data/local/tmp/xclipse940/ssbo_compute_parent.log", "w");
