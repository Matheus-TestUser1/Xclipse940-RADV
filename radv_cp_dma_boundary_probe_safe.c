/*
 * Xclipse 940 RADV - 4092-byte CP-DMA boundary probe
 *
 * Tests:
 *   HAL -> Instance -> PhysicalDevice -> Device -> Graphics Queue
 *   -> 2 x 4092-byte host-visible/coherent buffers
 *   -> CPU fills SRC with a pattern, clears DST
 *   -> vkCmdCopyBuffer(SRC -> DST), 4092 bytes (< 4096 threshold)
...
run_child(const char *driver_path, uint64_t fence_timeout_ns)
{
   ...
   const VkDeviceSize COPY_SIZE = 4092;

   probe_log = fopen("/data/local/tmp/xclipse940/cp_dma_boundary_probe.log", "w");
   if (probe_log) {
      setvbuf(probe_log, NULL, _IONBF, 0);
      persist_mark("START cp_dma_boundary_probe size=4092");
   }

   printf("=== Xclipse 940 RADV GPU buffer-copy probe ===\n");
   printf("driver: %s\n", driver_path);
   printf("GPU work: one 4092-byte vkCmdCopyBuffer (4 bytes below RADV 4096-byte compute threshold)\n");
...
#define GET_DEV(name) \
   ...
   printf("[11/15] Recorded one vkCmdCopyBuffer(4092 bytes), end=%d (%s)\n",
          vr, vk_result_name(vr));
   persist_mark("vkEndCommandBuffer result=%d", vr);
   if (vr != VK_SUCCESS)
      return 45;
   ...
   printf("\nPASS: GPU copied 4092 bytes SRC -> DST and CPU verification matched.\n");
