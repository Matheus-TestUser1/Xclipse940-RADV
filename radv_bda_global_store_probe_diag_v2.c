...
run_child(const char *driver_path, uint64_t fence_timeout_ns)
{
   ...
   probe_log = fopen("/data/local/tmp/xclipse940/bda_global_store_probe_v2.log", "w");
   if (probe_log) {
      setvbuf(probe_log, NULL, _IONBF, 0);
      persist_mark("START bda_global_store_probe");
   }

   printf("=== Xclipse 940 RADV BDA/global-store probe v2 ===\n");
   printf("driver: %s\n", driver_path);
...
#define GET_DEV(name) \
   ...
   persist_mark("before compute->host barrier");
   vkCmdPipelineBarrier(cb,
                        ...
                        0, NULL,
                        0, NULL);
   persist_mark("after compute->host barrier");
   ...
   printf("[17/17] CPU result = 0x%08x\n", got);
   persist_mark("CPU result=0x%08x", got);

   if (got != 0x12345678u) {
      printf("FAIL: BDA/global store completed but value did not match.\n");
      persist_mark("verification FAIL");
      return 70;
   }

   printf("PASS: direct BDA/global store wrote 0x12345678.\n");
   persist_mark("PASS");
...
main(int argc, char **argv)
{
   ...
   for (;;) {
      ...
      if (r == child) {
         FILE *f = fopen("/data/local/tmp/xclipse940/bda_global_store_parent_v2.log", "w");
