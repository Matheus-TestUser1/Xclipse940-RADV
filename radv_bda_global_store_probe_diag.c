...
run_child(const char *driver_path, uint64_t fence_timeout_ns)
{
   ...
   probe_log = fopen("/data/local/tmp/xclipse940/bda_global_store_probe.log", "w");
   if (probe_log) {
      setvbuf(probe_log, NULL, _IONBF, 0);
      persist_mark("START bda_global_store_probe");
   }

   printf("=== Xclipse 940 RADV BDA/global-store probe ===\n");
   printf("driver: %s\n", driver_path);
...
main(int argc, char **argv)
{
   ...
   for (;;) {
      ...
      if (r == child) {
         FILE *f = fopen("/data/local/tmp/xclipse940/bda_global_store_parent.log", "w");
