/*
 * Xclipse 940 RADV - minimal empty-submit probe
 *
 * Tests:
...
run_child(const char *driver_path, uint64_t fence_timeout_ns)
{
   ...
   printf("=== Xclipse 940 RADV minimal empty-submit probe ===\n");
   printf("driver: %s\n", driver_path);
   printf("GPU work: EMPTY COMMAND BUFFER ONLY\n");
...
#define GET_INST(name) \
   ...
   printf("       selected graphics queue family = %u\n", selected_qf);
...
#define GET_DEV(name) \
   ...
   printf("\nPASS: empty command buffer submitted and fence signaled.\n");
