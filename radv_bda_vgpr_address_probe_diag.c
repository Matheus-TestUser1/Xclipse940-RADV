 ...
 * void main() {
 *    uint64_t effective = pc.address + uint64_t(gl_GlobalInvocationID.x) * 4ul;
 *    *(PhysicalStorageBuffer uint *)effective = 0x12345678u;
 * }
 *
 * The dispatch remains 1x1x1, therefore gl_GlobalInvocationID.x == 0 and
 * the runtime target address is unchanged. The purpose is to force the
 * address through a divergent VGPR64 path in ACO instead of the usual
 * SGPR64 base + VGPR32 offset GLOBAL addressing mode.
 */
static const uint32_t bda_store_spv[] = {
   ...
   0x00000004, 0x00000000, 0x00050036, 0x00000002, 0x00000001, 0x00000000,
   ...
   0x00000015, 0x00050084, 0x00000005, 0x00000017, 0x00000016, 0x00000010,
   0x00050080, 0x00000005, 0x00000018, 0x00000013, 0x00000017, 0x0004007c,
...
run_child(const char *driver_path, uint64_t fence_timeout_ns)
{
   ...
   printf("driver: %s\n", driver_path);
   printf("work: one PhysicalStorageBuffer uint store = 0x12345678 (VGPR64-address diagnostic)\n");
